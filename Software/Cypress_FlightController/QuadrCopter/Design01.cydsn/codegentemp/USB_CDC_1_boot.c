...
static uint8  USB_CDC_1_started = 0u;


/*******************************************************************************
* Function Name: USB_CDC_1_CyBtldrCommStart
****************************************************************************//**
*
...
void USB_CDC_1_CyBtldrCommStart(void) 
{
    ...
    USB_CDC_1_Start(0u, USB_CDC_1_DWR_POWER_OPERATION);

    /* USB component started, the correct enumeration will be checked in the first Read operation. */
    USB_CDC_1_started = 1u;
}


/*******************************************************************************
* Function Name: USB_CDC_1_CyBtldrCommStop.
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: USB_CDC_1_CyBtldrCommReset.
****************************************************************************//**
*
...
{
    USB_CDC_1_EnableOutEP(USB_CDC_1_BTLDR_OUT_EP); 
}


/*******************************************************************************
* Function Name: USB_CDC_1_CyBtldrCommWrite.
****************************************************************************//**
*
                                                            ...
                                                            
{
    ...
    USB_CDC_1_LoadInEP(USB_CDC_1_BTLDR_IN_EP, pData, USB_CDC_1_BTLDR_SIZEOF_READ_BUFFER);

    /* Wait unitl host reads data from IN endpoint. */
    while ((USB_CDC_1_GetEPState(USB_CDC_1_BTLDR_IN_EP) == USB_CDC_1_IN_BUFFER_FULL) &&
           (0u != timeoutMs))
    {
        CyDelay(USB_CDC_1_BTLDR_WAIT_1_MS);
        timeoutMs--;
    }

    if (USB_CDC_1_GetEPState(USB_CDC_1_BTLDR_IN_EP) == USB_CDC_1_IN_BUFFER_FULL)
    {
...
/*******************************************************************************
* Function Name: USB_CDC_1_CyBtldrCommRead.
****************************************************************************//**
*
                                                            ...
                                                            
{
    ...
    if (size > USB_CDC_1_BTLDR_SIZEOF_WRITE_BUFFER)
    {
        size = USB_CDC_1_BTLDR_SIZEOF_WRITE_BUFFER;
    }

    /* Wait for enumeration first time. */
    if (0u != USB_CDC_1_started)
    {
        /* Wait for device enumeration. */
        while ((0u == USB_CDC_1_GetConfiguration()) && (0u != timeoutMs))
        {
            CyDelay(USB_CDC_1_BTLDR_WAIT_1_MS);
            timeoutMs--;
        }
        ...
        {
            (void) USB_CDC_1_IsConfigurationChanged();  /* Clear configuration changes state status. */
            USB_CDC_1_CyBtldrCommReset();
            
            USB_CDC_1_started = 0u;
        }
    }
    else /* Check for configuration changes, has been done by Host. */
    {
        if (0u != USB_CDC_1_IsConfigurationChanged()) /* Host could send double SET_INTERFACE request or RESET. */
        {
            if (0u != USB_CDC_1_GetConfiguration())   /* Init OUT endpoints when device reconfigured. */
            {
                USB_CDC_1_CyBtldrCommReset();
            }
        }
    ...
    while ((USB_CDC_1_GetEPState(USB_CDC_1_BTLDR_OUT_EP) != USB_CDC_1_OUT_BUFFER_FULL) && \
           (0u != timeoutMs))
    {
        CyDelay(USB_CDC_1_BTLDR_WAIT_1_MS);
        timeoutMs--;
    ...
    if (USB_CDC_1_GetEPState(USB_CDC_1_BTLDR_OUT_EP) == USB_CDC_1_OUT_BUFFER_FULL)
    {
        *count = USB_CDC_1_ReadOutEP(USB_CDC_1_BTLDR_OUT_EP, pData, size);
        retCode = CYRET_SUCCESS;
    }
    else
    {
