 ...
 * USB_CDC_1_GetDTERate(), USB_CDC_1_GetCharFormat(),
 * USB_CDC_1_GetParityType(), and USB_CDC_1_GetDataBits() APIs.
...
volatile uint8 USB_CDC_1_linesCoding[USB_CDC_1_MAX_MULTI_COM_NUM][USB_CDC_1_LINE_CODING_SIZE] =
{
    /*COM Port 1*/
    {
...
/**Used as a flag for the USB_CDC_1_IsLineChanged() API, to inform it that the
 ...
 * support. In case of 1 COM port, data is in 0 element.*/
volatile uint8  USB_CDC_1_linesChanged[USB_CDC_1_MAX_MULTI_COM_NUM];
 ...
 * USB_CDC_1_GetLineControl() API. It is an array of 2 elements for COM 
 * port 1 and COM port 2 for MultiCOM port support. In case of 1 COM port, data 
 * is in 0 element.*/
volatile uint16 USB_CDC_1_linesControlBitmap[USB_CDC_1_MAX_MULTI_COM_NUM];
/** Contains the 16-bit serial state value that was sent using the 
 * \ref USB_CDC_1_SendSerialState() API. . It is an array of 2 elements 
 * for COM port 1 and COM port 2 for MultiCOM port support. In case of 1 COM 
 * port, data is in 0 element.*/
volatile uint16 USB_CDC_1_serialStateBitmap[USB_CDC_1_MAX_MULTI_COM_NUM];
...
volatile uint8  USB_CDC_1_cdcDataInEp[USB_CDC_1_MAX_MULTI_COM_NUM];
 ...
 * port 2 for MultiCOM port support. In case of 1 COM port, data is in 0 element.*/
volatile uint8  USB_CDC_1_cdcDataOutEp[USB_CDC_1_MAX_MULTI_COM_NUM];
...
volatile uint8  USB_CDC_1_cdcCommInInterruptEp[USB_CDC_1_MAX_MULTI_COM_NUM];

/*PRIVATE*/

#define USB_CDC_1_CDC_IN_EP      (0u)
#define USB_CDC_1_CDC_OUT_EP     (1u)
#define USB_CDC_1_CDC_NOTE_EP    (2u)

#define USB_CDC_1_CDC_EP_MASK    (0x01u)

#define USB_CDC_1_GET_EP_COM_NUM(cdcComNums, epType) (((cdcComNums) >> (epType)) & USB_CDC_1_CDC_EP_MASK)


/***************************************
*     Static Function Prototypes
***************************************/
#if (USB_CDC_1_ENABLE_CDC_CLASS_API != 0u)
    static uint16 USB_CDC_1_StrLen(const char8 string[]) ;
    static t_USB_CDC_1_cdc_notification USB_CDC_1_serialStateNotification =
    {

        USB_CDC_1_SERIAL_STATE_REQUEST_TYPE, /* bRequestType    */
        USB_CDC_1_SERIAL_STATE,              /* bNotification   */
        0u,                                         /* wValue          */
        0u,                                         /* wValueMSB       */
        0u,                                         /* wIndex          */
        0u,                                         /* wIndexMSB       */
        USB_CDC_1_SERIAL_STATE_LENGTH,       /* wLength         */
        0u,                                         /* wLengthMSB      */
        0u,                                         /* wSerialState    */
        0u,                                         /* wSerialStateMSB */
    };
    static uint8 USB_CDC_1_activeCom = 0u;
#endif /* (USB_CDC_1_ENABLE_CDC_CLASS_API != 0u) */
...
/*******************************************************************************
* Function Name: USB_CDC_1_DispatchCDCClassRqst
****************************************************************************//**
*
...
*
* \globalvars
*   USB_CDC_1_linesCoding: Contains the current line coding structure.
...
*   USB_CDC_1_linesControlBitmap: Contains the current control signal
*     bitmap. It is set by the Host using SET_CONTROL_LINE request and returned
*     to the user code by the USBFS_GetLineControl() API.
*   USB_CDC_1_linesChanged: This variable is used as a flag for the
...
*
*******************************************************************************/
uint8 USB_CDC_1_DispatchCDCClassRqst(void) 
{
    uint8 requestHandled = USB_CDC_1_FALSE;
    uint8 comPort;

    comPort = USB_CDC_1_GetInterfaceComPort((uint8)USB_CDC_1_wIndexLoReg);
    ...
    if (0u != (USB_CDC_1_bmRequestTypeReg & USB_CDC_1_RQST_DIR_D2H))
    {
        ...
        {
            case USB_CDC_1_CDC_GET_LINE_CODING:
                USB_CDC_1_currentTD.count = USB_CDC_1_LINE_CODING_SIZE;
                USB_CDC_1_currentTD.pData = USB_CDC_1_linesCoding[comPort];
            ...
            #ifdef USB_CDC_1_DISPATCH_CDC_CLASS_CDC_READ_REQUESTS_CALLBACK
                requestHandled = USB_CDC_1_DispatchCDCClass_CDC_READ_REQUESTS_Callback();
            #endif /* (USB_CDC_1_DISPATCH_CDC_CLASS_CDC_READ_REQUESTS_CALLBACK) */
                break;
        }
    }
    else
    {
        ...
        switch (USB_CDC_1_bRequestReg)
        {
            case USB_CDC_1_CDC_SET_LINE_CODING:
                USB_CDC_1_currentTD.count  = USB_CDC_1_LINE_CODING_SIZE;
                USB_CDC_1_currentTD.pData  = USB_CDC_1_linesCoding[comPort];
                USB_CDC_1_linesChanged[comPort] |= USB_CDC_1_LINE_CODING_CHANGED;

                requestHandled = USB_CDC_1_InitControlWrite();
                break;

            case USB_CDC_1_CDC_SET_CONTROL_LINE_STATE:
                USB_CDC_1_linesControlBitmap[comPort] = (uint8) USB_CDC_1_wValueLoReg;
                USB_CDC_1_linesChanged[comPort]      |= USB_CDC_1_LINE_CONTROL_CHANGED;
            ...
            default:
                /* Do not handle this request unless callback is defined. */
            #ifdef USB_CDC_1_DISPATCH_CDC_CLASS_CDC_WRITE_REQUESTS_CALLBACK
                requestHandled = USB_CDC_1_DispatchCDCClass_CDC_WRITE_REQUESTS_Callback();
            #endif /* (USB_CDC_1_DISPATCH_CDC_CLASS_CDC_WRITE_REQUESTS_CALLBACK) */
                break;
        }
    }
...
/***************************************************************************
* Function Name: USB_CDC_1_GetInterfaceComPort
************************************************************************//**
*   \internal
...
*
***************************************************************************/
uint8 USB_CDC_1_GetInterfaceComPort(uint8 interface) 
{
    uint8 comPort = 0u;
    uint8 res = 0xFFu;
    uint8 notEp;

    while (comPort < USB_CDC_1_MAX_MULTI_COM_NUM)
    {
        notEp = USB_CDC_1_cdcCommInInterruptEp[comPort];

        if (USB_CDC_1_EP[notEp].interface == interface)
        {
            res = comPort;
            comPort = USB_CDC_1_MAX_MULTI_COM_NUM;
        }
...
#if (USB_CDC_1_ENABLE_CDC_CLASS_API != 0u)
/***************************************************************************
* Function Name: USB_CDC_1_CDC_Init
************************************************************************//**
*
...
*   USB_CDC_1_SUCCESS   CDC interface was initialized correctly
*   USB_CDC_1_FAILURE   CDC interface was not initialized
*
* \globalvars
*   USB_CDC_1_linesChanged: Initialized to zero.
*   USB_CDC_1_cdcDataOutEp: Used as an OUT endpoint number.
*
* \reentrant
*  No.
*
*******************************************************************************/
uint8 USB_CDC_1_CDC_Init(void) 
{
    uint8 comPort;
    uint8 outEp;
    uint8 ret = USB_CDC_1_SUCCESS;

    USB_CDC_1_activeCom = 0u;
    USB_CDC_1_linesChanged[USB_CDC_1_COM_PORT1] = 0u;
    USB_CDC_1_linesChanged[USB_CDC_1_COM_PORT2] = 0u;

    for(comPort = 0u; comPort<USB_CDC_1_MAX_MULTI_COM_NUM; comPort++)
    {
        outEp = USB_CDC_1_cdcDataOutEp[comPort];
        if((0u != outEp) && (USB_CDC_1_MAX_EP > outEp))
        {
            USB_CDC_1_EnableOutEP(outEp);
        }

    }

    /* COM Port 1 should be correct to proceed. */
    if ((0u == USB_CDC_1_cdcDataInEp[USB_CDC_1_COM_PORT1]) \
            || (0u == USB_CDC_1_cdcDataOutEp[USB_CDC_1_COM_PORT1]) \
            || (0u ==  USB_CDC_1_cdcCommInInterruptEp[USB_CDC_1_COM_PORT1])
            || (USB_CDC_1_cdcDataInEp[USB_CDC_1_COM_PORT1] >= USB_CDC_1_MAX_EP)
            || (USB_CDC_1_cdcDataOutEp[USB_CDC_1_COM_PORT1] >= USB_CDC_1_MAX_EP)
            || (USB_CDC_1_cdcCommInInterruptEp[USB_CDC_1_COM_PORT1] >= USB_CDC_1_MAX_EP))
    {
        ret = USB_CDC_1_FAILURE;
    }
...
/*******************************************************************************
* Function Name: USB_CDC_1_PutData
****************************************************************************//**
*
*  This function sends a specified number of bytes from the location specified
*  by a pointer to the PC. The USB_CDC_1_CDCIsReady() function should be
...
*  identified. To send zero-length packet, use USB_CDC_1_PutData() API
...
* \globalvars
*
*   USB_CDC_1_cdcDataInEp: CDC IN endpoint number used for sending
*     data.
*
* \reentrant
*  No.
*
*******************************************************************************/
void USB_CDC_1_PutData(const uint8* pData, uint16 length) 
{
    uint8 epNumber = USB_CDC_1_cdcDataInEp[USB_CDC_1_activeCom];

    /* Limit length to maximum packet size for endpoint. */
    if (length > USB_CDC_1_EP[epNumber].bufferSize)
    {
        /* Caution: Data will be lost if length is greater than Max Packet Size. */
        length = USB_CDC_1_EP[epNumber].bufferSize;

        /* Halt CPU in debug mode */
        CYASSERT(0u != 0u);
    }

    USB_CDC_1_LoadInEP(epNumber, pData, length);
}


/*******************************************************************************
* Function Name: USB_CDC_1_StrLen
****************************************************************************//**
*
...
/***************************************************************************
* Function Name: USB_CDC_1_PutString
************************************************************************//**
*
...
*  The USB_CDC_1_CDCIsReady() function should be called before
*  sending data with a new call to USB_CDC_1_PutString(), to be sure
...
*
***************************************************************************/
void USB_CDC_1_PutString(const char8 string[]) 
{
    uint16 strLength;
    uint16 sendLength;
    uint16 bufIndex = 0u;
    ...
    strLength = USB_CDC_1_StrLen(string);

    do
    {
        /* Limit length to maximum packet size of endpoint. */
        sendLength = (strLength > USB_CDC_1_EP[epNumber].bufferSize) ?
                                  USB_CDC_1_EP[epNumber].bufferSize : strLength;

        /* Load IN endpoint and expose it to host. */
        USB_CDC_1_LoadInEP(epNumber, (const uint8 *)&string[bufIndex], sendLength);
        strLength -= sendLength;

        /* If more data are present to send or full packet was sent */
        if ((strLength > 0u) || (sendLength == USB_CDC_1_EP[epNumber].bufferSize))
        {
            bufIndex += sendLength;

            /* Wait until host read data from IN endpoint buffer. */
            while (USB_CDC_1_IN_BUFFER_FULL == USB_CDC_1_EP[epNumber].apiEpState)
            {
            }
            ...
            */
            if (0u == strLength)
            {
                USB_CDC_1_LoadInEP(epNumber, NULL, 0u);
            }
        }
    }
    while (strLength > 0u);
...
/***************************************************************************
* Function Name: USB_CDC_1_PutChar
************************************************************************//**
*
...
*
***************************************************************************/
void USB_CDC_1_PutChar(char8 txDataByte) 
{
    uint8 dataByte;
    dataByte = (uint8) txDataByte;

    USB_CDC_1_LoadInEP(USB_CDC_1_cdcDataInEp[USB_CDC_1_activeCom], &dataByte, 1u);
}


/*******************************************************************************
* Function Name: USB_CDC_1_PutCRLF
****************************************************************************//**
*
...
*
*******************************************************************************/
void USB_CDC_1_PutCRLF(void) 
{
    const uint8 CYCODE txData[] = {0x0Du, 0x0Au};

    USB_CDC_1_LoadInEP(USB_CDC_1_cdcDataInEp[USB_CDC_1_activeCom], (const uint8 *)txData, 2u);
}


/*******************************************************************************
* Function Name: USB_CDC_1_GetCount
****************************************************************************//**
*
*  This function returns the number of bytes that were received from the PC.
*  The returned length value should be passed to USB_CDC_1_GetData() as
*  a parameter to read all received data. If all of the received data is not
*  read at one time by the USB_CDC_1_GetData() API, the unread data will
*  be lost.
*
* \return
...
*
* \globalvars
*   USB_CDC_1_cdcDataOutEp: CDC OUT endpoint number used.
*
*******************************************************************************/
uint16 USB_CDC_1_GetCount(void) 
{
    uint16 bytesCount;

    uint8  epNumber = USB_CDC_1_cdcDataOutEp[USB_CDC_1_activeCom];

    if (USB_CDC_1_OUT_BUFFER_FULL == USB_CDC_1_EP[epNumber].apiEpState)
    {
        bytesCount = USB_CDC_1_GetEPCount(epNumber);
    }
    else
    {
        bytesCount = 0u;
...
/*******************************************************************************
* Function Name: USB_CDC_1_DataIsReady
****************************************************************************//**
*
*  This function returns a non-zero value if the component received data or
*  received zero-length packet. The USB_CDC_1_GetAll() or
*  USB_CDC_1_GetData() API should be called to read data from the buffer
...
*
*******************************************************************************/
uint8 USB_CDC_1_DataIsReady(void) 
{
    return (USB_CDC_1_GetEPState(USB_CDC_1_cdcDataOutEp[USB_CDC_1_activeCom]));
}


/*******************************************************************************
* Function Name: USB_CDC_1_CDCIsReady
****************************************************************************//**
*
...
*  USB_CDC_1_PutData(),USB_CDC_1_PutString(),
*  USB_CDC_1_PutChar or USB_CDC_1_PutCRLF(),
...
*
* \globalvars
*   USB_CDC_1_cdcDataInEp: CDC IN endpoint number used.
*
*******************************************************************************/
uint8 USB_CDC_1_CDCIsReady(void) 
{
    return (USB_CDC_1_GetEPState(USB_CDC_1_cdcDataInEp[USB_CDC_1_activeCom]));
}


/***************************************************************************
* Function Name: USB_CDC_1_GetData
************************************************************************//**
*
...
*  The USB_CDC_1_DataIsReady() API should be called first, to be sure
*  that data is received from the host. If all received data will not be read at
*  once, the unread data will be lost. The USB_CDC_1_GetData() API should
...
*
***************************************************************************/
uint16 USB_CDC_1_GetData(uint8* pData, uint16 length) 
{
    uint8 epNumber = USB_CDC_1_cdcDataOutEp[USB_CDC_1_activeCom];

    /* Read data from OUT endpoint buffer. */
    length = USB_CDC_1_ReadOutEP(epNumber, pData, length);

#if (USB_CDC_1_EP_MANAGEMENT_DMA_MANUAL)
    /* Wait until DMA complete transferring data from OUT endpoint buffer. */
    while (USB_CDC_1_OUT_BUFFER_FULL == USB_CDC_1_GetEPState(epNumber))
    {
    }
...
*
* \globalvars
*   - \ref USB_CDC_1_cdcDataOutEp: CDC OUT endpoint number used.
*   - \ref USB_CDC_1_EP[].bufferSize: EP max packet size is used as a
...
*
*******************************************************************************/
uint16 USB_CDC_1_GetAll(uint8* pData) 
{
    uint8 epNumber = USB_CDC_1_cdcDataOutEp[USB_CDC_1_activeCom];
    uint16 dataLength;

    /* Read data from OUT endpoint buffer. */
    dataLength = USB_CDC_1_ReadOutEP(epNumber, pData, USB_CDC_1_EP[epNumber].bufferSize);
...
/***************************************************************************
* Function Name: USB_CDC_1_GetChar
************************************************************************//**
*
...
*
***************************************************************************/
uint8 USB_CDC_1_GetChar(void) 
{
     uint8 rxData;
     uint8 epNumber = USB_CDC_1_cdcDataOutEp[USB_CDC_1_activeCom];

    (void) USB_CDC_1_ReadOutEP(epNumber, &rxData, 1u);
...
/*******************************************************************************
* Function Name: USB_CDC_1_IsLineChanged
****************************************************************************//**
*
...
*  device. The USB_CDC_1_GetDTERate(), USB_CDC_1_GetCharFormat()
*  or USB_CDC_1_GetParityType() or USB_CDC_1_GetDataBits() API
*  should be called to read data coding information.
*  The USB_CDC_1_GetLineControl() API should be called to read line
*  control information.
*
* \return
...
*
* \globalvars
*  - \ref USB_CDC_1_transferState: it is checked to be sure then OUT
*    data phase has been complete, and data written to the lineCoding or
*    Control Bitmap buffer.
*  - \ref USB_CDC_1_linesChanged: used as a flag to be aware that
*    Host has been sent request for changing Line Coding or Control Bitmap.
*
*******************************************************************************/
uint8 USB_CDC_1_IsLineChanged(void) 
{
    uint8 state = 0u;

    /* transferState is checked to be sure then OUT data phase has been complete */
    if (USB_CDC_1_transferState == USB_CDC_1_TRANS_STATE_IDLE)
    {
        if (USB_CDC_1_linesChanged[USB_CDC_1_activeCom] != 0u)
        {
            state = USB_CDC_1_linesChanged[USB_CDC_1_activeCom];
            USB_CDC_1_linesChanged[USB_CDC_1_activeCom] = 0u;
        }
    }
...
/***************************************************************************
* Function Name: USB_CDC_1_GetDTERate
************************************************************************//**
*
...
*
* \globalvars
*  USB_CDC_1_linesCoding: First four bytes converted to uint32
*    depend on compiler, and returned as a data rate.
*
*******************************************************************************/
uint32 USB_CDC_1_GetDTERate(void) 
{
    uint32 rate;

    rate = USB_CDC_1_linesCoding[USB_CDC_1_activeCom][USB_CDC_1_LINE_CODING_RATE + 3u];
    rate = (rate << 8u) | USB_CDC_1_linesCoding[USB_CDC_1_activeCom][USB_CDC_1_LINE_CODING_RATE + 2u];
    rate = (rate << 8u) | USB_CDC_1_linesCoding[USB_CDC_1_activeCom][USB_CDC_1_LINE_CODING_RATE + 1u];
    rate = (rate << 8u) | USB_CDC_1_linesCoding[USB_CDC_1_activeCom][USB_CDC_1_LINE_CODING_RATE];
...
/*******************************************************************************
* Function Name: USB_CDC_1_GetCharFormat
****************************************************************************//**
*
...
*
* \globalvars
*  USB_CDC_1_linesCoding: used to get a parameter.
*
*******************************************************************************/
uint8 USB_CDC_1_GetCharFormat(void) 
{
    return (USB_CDC_1_linesCoding[USB_CDC_1_activeCom][USB_CDC_1_LINE_CODING_STOP_BITS]);
}


/*******************************************************************************
* Function Name: USB_CDC_1_GetParityType
****************************************************************************//**
*
...
*
*******************************************************************************/
uint8 USB_CDC_1_GetParityType(void) 
{
    return (USB_CDC_1_linesCoding[USB_CDC_1_activeCom][USB_CDC_1_LINE_CODING_PARITY]);
}


/***************************************************************************
* Function Name: USB_CDC_1_GetDataBits
************************************************************************//**
*
...
*
*******************************************************************************/
uint8 USB_CDC_1_GetDataBits(void) 
{
    return (USB_CDC_1_linesCoding[USB_CDC_1_activeCom][USB_CDC_1_LINE_CODING_DATA_BITS]);
}


/***************************************************************************
* Function Name: USB_CDC_1_GetLineControl
************************************************************************//**
*
...
*
* \globalvars
*  USB_CDC_1_linesControlBitmap: used to get a parameter.
*
*******************************************************************************/
uint16 USB_CDC_1_GetLineControl(void) 
{
    return (USB_CDC_1_linesControlBitmap[USB_CDC_1_activeCom]);
}


/*******************************************************************************
* Function Name: USB_CDC_1_SendSerialState
****************************************************************************//**
*
...
*  USB_CDC_1_NotificationIsReady() API must be called to check if the
...
*
*******************************************************************************/
void USB_CDC_1_SendSerialState (uint16 serialState) 
{
    uint8 epNumber = USB_CDC_1_cdcCommInInterruptEp[USB_CDC_1_activeCom];

    if(USB_CDC_1_SERIAL_STATE_SIZE <= USB_CDC_1_EP[epNumber].bufferSize)
    {
        /* Save current SERIAL_STATE bitmap. */
        USB_CDC_1_serialStateBitmap[USB_CDC_1_activeCom] = serialState;

        /* Add interface number */
        USB_CDC_1_serialStateNotification.wIndex = USB_CDC_1_EP[epNumber].interface;

        /*Form SERIAL_STATE data*/
        USB_CDC_1_serialStateNotification.wSerialState =    LO8(USB_CDC_1_serialStateBitmap[USB_CDC_1_activeCom]);
        USB_CDC_1_serialStateNotification.wSerialStateMSB = HI8(USB_CDC_1_serialStateBitmap[USB_CDC_1_activeCom]);

        USB_CDC_1_LoadInEP(epNumber, (uint8 *) &USB_CDC_1_serialStateNotification, sizeof(USB_CDC_1_serialStateNotification));
    }
}


/*******************************************************************************
* Function Name: USB_CDC_1_GetSerialState
****************************************************************************//**
*
...
*
*******************************************************************************/
uint16 USB_CDC_1_GetSerialState(void) 
{
    return USB_CDC_1_serialStateBitmap[USB_CDC_1_activeCom];
}


/*******************************************************************************
* Function Name: USB_CDC_1_NotificationIsReady
****************************************************************************//**
*
...
*  USB_CDC_1_SendSerialState() to ensure that any previous
...
*
*******************************************************************************/
uint8 USB_CDC_1_NotificationIsReady(void) 
{
    return (USB_CDC_1_EP[USB_CDC_1_cdcCommInInterruptEp[USB_CDC_1_activeCom]].apiEpState);
}


/*******************************************************************************
* Function Name: USB_CDC_1_SetComPort
****************************************************************************//**
*
...
*
*******************************************************************************/
void USB_CDC_1_SetComPort(uint8 comNumber) 
{
    if ((USB_CDC_1_activeCom != comNumber) && \
            (comNumber < USB_CDC_1_MAX_MULTI_COM_NUM ))
    {
        USB_CDC_1_activeCom = comNumber;
    }
}


/*******************************************************************************
* Function Name: USB_CDC_1_GetComPort
****************************************************************************//**
*
...
*
*******************************************************************************/
uint8 USB_CDC_1_GetComPort(void) 
{
    return (USB_CDC_1_activeCom);
}


#endif  /* (USB_CDC_1_ENABLE_CDC_CLASS_API) */


/***************************************************************************
* Function Name: USB_CDC_1_Cdc_EpInit
************************************************************************//**
*
*  \internal
...
*   USB_CDC_1_cdcDataInEp[], USB_CDC_1_cdcCommInInterruptEp[],
*   USB_CDC_1_cdcDataOutEp[]
*
...
*
***************************************************************************/
uint8 USB_CDC_1_Cdc_EpInit(const T_USB_CDC_1_EP_SETTINGS_BLOCK CYCODE *pEP, uint8 epNum, uint8 cdcComNums) 
{
    uint8 epType;
    ...
    {
        if (epType != USB_CDC_1_EP_TYPE_INT)
        {
            USB_CDC_1_cdcDataInEp[USB_CDC_1_GET_EP_COM_NUM(cdcComNums, USB_CDC_1_CDC_IN_EP)] = epNum;
            cdcComNums |= (uint8)(USB_CDC_1_COM_PORT2 << USB_CDC_1_CDC_IN_EP);
        }
        else
        {

            USB_CDC_1_cdcCommInInterruptEp[USB_CDC_1_GET_EP_COM_NUM(cdcComNums, USB_CDC_1_CDC_NOTE_EP)] = epNum;
            cdcComNums |= (uint8)(USB_CDC_1_COM_PORT2 << USB_CDC_1_CDC_NOTE_EP);
        }
    }
    else
    {
        if (epType != USB_CDC_1_EP_TYPE_INT)
        {
            USB_CDC_1_cdcDataOutEp[USB_CDC_1_GET_EP_COM_NUM(cdcComNums, USB_CDC_1_CDC_OUT_EP)] = epNum;
            cdcComNums |= (uint8)(USB_CDC_1_COM_PORT2 << USB_CDC_1_CDC_OUT_EP);
        }
    }
    return (cdcComNums);
}
