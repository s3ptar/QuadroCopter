...
volatile uint8 USB_CDC_1_hidProtocol[USB_CDC_1_MAX_INTERFACES_NUMBER];
 ...
 * reload timer.*/
volatile uint8 USB_CDC_1_hidIdleRate[USB_CDC_1_MAX_INTERFACES_NUMBER];

/** This variable contains the timer counter, which is decremented and reloaded 
 * by the USBFS_UpdateHIDTimer() API.*/
volatile uint8 USB_CDC_1_hidIdleTimer[USB_CDC_1_MAX_INTERFACES_NUMBER]; /* HID device idle rate value */
...
/*******************************************************************************
* Function Name: USB_CDC_1_UpdateHIDTimer
****************************************************************************//**
*
...
*
*******************************************************************************/
uint8 USB_CDC_1_UpdateHIDTimer(uint8 interface) 
{
    uint8 stat = USB_CDC_1_IDLE_TIMER_INDEFINITE;

    if(USB_CDC_1_hidIdleRate[interface] != 0u)
    {
        if(USB_CDC_1_hidIdleTimer[interface] > 0u)
        {
            USB_CDC_1_hidIdleTimer[interface]--;
            stat = USB_CDC_1_IDLE_TIMER_RUNNING;
        }
        else
        {
            USB_CDC_1_hidIdleTimer[interface] = USB_CDC_1_hidIdleRate[interface];
            stat = USB_CDC_1_IDLE_TIMER_EXPIRED;
        }
    }
...
/*******************************************************************************
* Function Name: USB_CDC_1_GetProtocol
****************************************************************************//**
*
...
*
*******************************************************************************/
uint8 USB_CDC_1_GetProtocol(uint8 interface) 
{
    return(USB_CDC_1_hidProtocol[interface]);
}


/*******************************************************************************
* Function Name: USB_CDC_1_DispatchHIDClassRqst
****************************************************************************//**
*
...
uint8 USB_CDC_1_DispatchHIDClassRqst(void) 
{
    ...
    uint8 interfaceNumber = (uint8) USB_CDC_1_wIndexLoReg;
    ...
    if (0u != (USB_CDC_1_bmRequestTypeReg & USB_CDC_1_RQST_DIR_D2H))
    {
        ...
        switch (USB_CDC_1_bRequestReg)
        {
            case USB_CDC_1_GET_DESCRIPTOR:
                if (USB_CDC_1_wValueHiReg == USB_CDC_1_DESCR_HID_CLASS)
                {
                    USB_CDC_1_FindHidClassDecriptor();
                    if (USB_CDC_1_currentTD.count != 0u)
                    {
                        requestHandled = USB_CDC_1_InitControlRead();
                    }
                }
                else if (USB_CDC_1_wValueHiReg == USB_CDC_1_DESCR_HID_REPORT)
                {
                    USB_CDC_1_FindReportDescriptor();
                    if (USB_CDC_1_currentTD.count != 0u)
                    {
                        requestHandled = USB_CDC_1_InitControlRead();
                    }
            ...
            case USB_CDC_1_HID_GET_REPORT:
                USB_CDC_1_FindReport();
                if (USB_CDC_1_currentTD.count != 0u)
                {
                    requestHandled = USB_CDC_1_InitControlRead();
                }
            ...
            case USB_CDC_1_HID_GET_IDLE:
                ...
                if ((interfaceNumber < USB_CDC_1_MAX_INTERFACES_NUMBER) && (USB_CDC_1_wValueLoReg == 0u)) 
                {
                    USB_CDC_1_currentTD.count = 1u;
                    USB_CDC_1_currentTD.pData = &USB_CDC_1_hidIdleRate[interfaceNumber];
                    requestHandled  = USB_CDC_1_InitControlRead();
                }
                break;
                
            case USB_CDC_1_HID_GET_PROTOCOL:
                /* Validate interfaceNumber */
                if( interfaceNumber < USB_CDC_1_MAX_INTERFACES_NUMBER)
                {
                    USB_CDC_1_currentTD.count = 1u;
                    USB_CDC_1_currentTD.pData = &USB_CDC_1_hidProtocol[interfaceNumber];
                    requestHandled  = USB_CDC_1_InitControlRead();
                }
                break;
    ...
    {   
        ...
        switch (USB_CDC_1_bRequestReg)
        {
            ...
            case USB_CDC_1_HID_SET_IDLE:
                /* This function does not support multiple reports per interface */
                /* Validate interfaceNumber and Report ID (should be 0): Do not support Idle per Report ID */
                if ((interfaceNumber < USB_CDC_1_MAX_INTERFACES_NUMBER) && (USB_CDC_1_wValueLoReg == 0u))
                {
                    USB_CDC_1_hidIdleRate[interfaceNumber] = (uint8)USB_CDC_1_wValueHiReg;
                    ...
                    */
                    if(USB_CDC_1_hidIdleRate[interfaceNumber] <
                       USB_CDC_1_hidIdleTimer[interfaceNumber])
                    {
                        /* Set the timer to zero and let the UpdateHIDTimer() API return IDLE_TIMER_EXPIRED status*/
                        USB_CDC_1_hidIdleTimer[interfaceNumber] = 0u;
                    }
                    ...
                    */
                    else if(USB_CDC_1_hidIdleTimer[interfaceNumber] <= 1u)
                    {
                        /* Do nothing.
                    ...
                    {   /* Reload the timer*/
                        USB_CDC_1_hidIdleTimer[interfaceNumber] =
                        USB_CDC_1_hidIdleRate[interfaceNumber];
                    }
                    requestHandled = USB_CDC_1_InitNoDataControlTransfer();
                }
                break;

            case USB_CDC_1_HID_SET_PROTOCOL:
                /* Validate interfaceNumber and protocol (must be 0 or 1) */
                if ((interfaceNumber < USB_CDC_1_MAX_INTERFACES_NUMBER) && (USB_CDC_1_wValueLoReg <= 1u))
                {
                    USB_CDC_1_hidProtocol[interfaceNumber] = (uint8)USB_CDC_1_wValueLoReg;
                    requestHandled = USB_CDC_1_InitNoDataControlTransfer();
                }
                break;
...
void USB_CDC_1_FindHidClassDecriptor(void) 
{
    ...
    interfaceN = (uint8) USB_CDC_1_wIndexLoReg;
    ...
    pTmp = &pTmp[USB_CDC_1_interfaceSetting[interfaceN]];
    ...
    USB_CDC_1_currentTD.count = *pDescr;
    USB_CDC_1_currentTD.pData = pDescr;
}
...
void USB_CDC_1_FindReportDescriptor(void) 
{
    ...
    USB_CDC_1_currentTD.count =  ((uint16)((uint16) pDescr[1u] << 8u) | pDescr[0u]);
    USB_CDC_1_currentTD.pData = &pDescr[2u];
}


/*******************************************************************************
* Function Name: USB_CDC_1_FindReport
****************************************************************************//**
*
...
void USB_CDC_1_FindReport(void) 
{
...
#ifdef USB_CDC_1_FIND_REPORT_CALLBACK
    USB_CDC_1_FindReport_Callback();
#endif /* (USB_CDC_1_FIND_REPORT_CALLBACK) */
    
    USB_CDC_1_currentTD.count = 0u;   /* Init not supported condition */
    pTmp = USB_CDC_1_GetConfigTablePtr(USB_CDC_1_configuration - 1u);
    reportType = (uint8) USB_CDC_1_wValueHiReg;
    interfaceN = (uint8) USB_CDC_1_wIndexLoReg;
    ...
    /* USB_DEVICEx_CONFIGURATIONy_INTERFACEz_TABLE */
    pTmp = (const T_USB_CDC_1_LUT CYCODE *) pTmp->p_list;
    if (interfaceN < USB_CDC_1_MAX_INTERFACES_NUMBER)
    {
        /* Now use Alternate setting number */
        pTmp = &pTmp[USB_CDC_1_interfaceSetting[interfaceN]];
        ...
        if ((reportType >= USB_CDC_1_HID_GET_REPORT_INPUT) &&
            (reportType <= USB_CDC_1_HID_GET_REPORT_FEATURE))
        {
            /* Get the entry proper TD (IN, OUT or Feature Report Table)*/
            pTmp = &pTmp[reportType - 1u];
            
            /* Get reportID */
            reportType = (uint8) USB_CDC_1_wValueLoReg;
            
            /* Validate table support by the HID descriptor, compare table count with reportID */
            if (pTmp->c >= reportType)
            {
                pTD = (T_USB_CDC_1_TD *) pTmp->p_list;
                pTD = &pTD[reportType];                          /* select entry depend on report ID*/
                USB_CDC_1_currentTD.pData = pTD->pData;   /* Buffer pointer */
                USB_CDC_1_currentTD.count = pTD->count;   /* Buffer Size */
                USB_CDC_1_currentTD.pStatusBlock = pTD->pStatusBlock;
            }
        }
    }
}
